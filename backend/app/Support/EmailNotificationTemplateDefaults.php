...
final class EmailNotificationTemplateDefaults
{
    ...
    public static function templates(): array
    {
        return [
            ...
            self::email('100030', '上游余额不足提醒', '上游供应商余额低于告警阈值时通知管理员。', '【{{site_name}}】上游余额不足提醒 - {{supplier_name}}', '{{recipient_name}}，上游供应商余额已低于告警阈值。', [['供应商', '{{supplier_name}}'], ['上游标识', '{{provider_key}}'], ['当前余额', '{{balance}} {{currency}}'], ['上次余额', '{{previous_balance}}'], ['告警阈值', '{{threshold}}'], ['同步时间', '{{synced_at}}']], '请尽快为该上游账户充值，余额耗尽会导致新订单开通失败。', ['recipient_name', 'supplier_name', 'provider_key', 'balance', 'previous_balance', 'threshold', 'currency', 'synced_at'], 'admin', '#b45309'),
            self::email('100031', '上游余额不足导致开通失败', '上游开通因余额或额度不足而失败时通知管理员。', '【{{site_name}}】上游开通失败（余额不足）- {{supplier_name}}', '{{recipient_name}}，有订单因上游余额不足开通失败。', [['供应商', '{{supplier_name}}'], ['当前余额', '{{balance}}'], ['告警阈值', '{{threshold}}'], ['订单号', '{{order_no}}'], ['产品名称', '{{product_name}}'], ['客户', '{{user_name}}'], ['失败原因', '{{error_message}}'], ['失败时间', '{{failed_at}}']], '请为上游账户充值后，在后台对该订单重新提交上游开通。', ['recipient_name', 'supplier_name', 'balance', 'threshold', 'order_no', 'product_name', 'user_name', 'error_message', 'failed_at'], 'admin', '#b91c1c'),
        ];
    }
