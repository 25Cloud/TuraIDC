...
import type { ServiceVncCredentials } from '../types';
...
export function useConsoleVnc(options: UseConsoleVncOptions) {
  const { userId, serviceId, actionLoading, activeTab } = options;
  ...
  async function requestVncUrl(targetWindow: Window | null = null): Promise<string> {
    const res = await clientApi.serviceVnc(serviceId.value);
