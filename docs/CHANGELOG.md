...
- **易支付网关标识统一为 epay**：插件目录 `backend/plugins/gateways/yi_pay` → `epay`、命名空间 `YiPay` → `Epay`、插件 slug/key 与业务网关编码 `yipay`/`yi_pay` → `epay`，对齐「目录名 = slug = key」的单标识约定；用户端支付记录筛选项同步。兼容处理：存量 `payments.gateway_key`/`payment_callbacks.gateway_key`/`gateway_logs.gateway_key` 由迁移改写为 `epay`（插件注册行保 id 改名，配置与绑定不丢）；`normalize()` 保留 `yipay`/`yi_pay` 别名，改名前下发给网关的回调地址 `/payment/notify/yipay` 无需网关侧改动即可继续入账
...
- **开放接口配置开关显示为关闭**：管理端「开放接口 → 配置」的三个开关（启用开放接口 / 必须绑定手机号 / 必须实名认证）把后端返回的数字 0/1 直接绑给 `t-switch`，而该组件未声明 `custom-value` 时以布尔 `true` 判定开启，`1 !== true` 导致开关恒显示关闭、切换后又把值写成布尔 `true`，保存成功也被回写拽回关闭。现按项目既有约定补 `:custom-value="[1, 0]"`，并补 e2e 用例钉住「按接口返回渲染 + 保存提交数字且不被拽回」
- **魔方财务自定义面板控制台修复**：`/console/capabilities` 跨对象读私有属性必然 500，改用公开代理方法；面板响应跳过点击劫持头并按白名单下发 `frame-ancestors`，解决 iframe 内「拒绝连接」；`module_button` 混入导致按钮误成 tab、安全组 tab 撞车，改为仅 `select=client_area` 成 tab；面板片段内嵌的转售 API 凭据（`Authorization: JWT`）会随 HTML 下发终端用户，改写时剥离并改走本地票据代理；上游静态资源改经 `console-area/asset` 反代并缓存 24h，不再暴露上游域名；为半页与整页两种面板注入 jQuery/Bootstrap/SweetAlert2 运行时（含 SweetAlert2 v11 `type`→`icon` 兼容垫片），修复面板按钮弹窗失效与「一条样式都没有」
- **面板加载态与监控图表标签**：iframe 内容未就绪时加载态不再提前消失（改用 `load` 事件驱动的覆盖式加载态），并修复加载圈被全局直角规则压成直线；监控图表标签改读上游 `title` 且不再对指标名做曲线名归一，消除 `cpu`/`cpu`/`内存`/`cpu` 退化
...
- **leaf 实名认证签名失败**：签名算法对齐官方 SDK 规范——签名串改为 `METHOD\nPATH\nTIMESTAMP\nNONCE\nBODY_SHA256`，时间戳使用毫秒 ISO8601（`.000Z`），签名 PATH 不含域名与 query；平台业务错误从 `{error:{code,message}}` 子对象展开映射为中文提示

### 新增

- **充值实名开关**：`finance.recharge_requires_verification`（默认开启，行为与既往一致），管理端设置页新增「账户与财务」分组可关闭，面向非大陆主体部署允许未实名用户充值
