...
- [从智简魔方财务系统迁移（实验）](../../references/database/migrate-from-zjmf-finance-experimental.md)：业务数据从魔方财务 `shd_` dump 导入 turaidc 的实验性迁移器（用户、商品、订单、账单等）。本文档聚焦上游绑定切换，数据迁移器聚焦业务数据导入，两者互补。
- [本地 IDC 数据迁移流程](../../references/database/local-idc-data-migration.md)：当前结构内 dump 迁移的正式流程。
