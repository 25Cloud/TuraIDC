...
| 2026-08-21 | 上传凭证校验默认放行（`upload_token_required=false`）+ 每日孤儿文件清理 | 部署上游系统无法同步配套修改，fail-closed 校验会拒绝回调附件上传（图片无法显示）；默认放行无凭证上传恢复功能，带凭证上传仍强制匹配；以每日清理任务（保留期 7 天、未引用即删）缓解磁盘占用。 |
