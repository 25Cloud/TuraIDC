...
9. 新增或修改代码必须遵循同一模块现有的命名、目录、分层、异常处理、响应结构和测试组织方式；确需引入新模式时，必须在对应设计或执行计划中说明原因，并同步更新相关规范。
10. 规则变化必须同步项目文档和 TuraIDC 相关 skills，避免后续代理和人工开发继续沿用旧约定。
...
触发以下变化时，必须同步项目文档和 TuraIDC 相关 skills：
