...
- [从智简魔方财务系统迁移（实验）](../../参考资料/数据库/从智简魔方财务系统迁移（实验）.md)：业务数据从魔方财务 `shd_` dump 导入 turaidc 的实验性迁移器（用户、商品、订单、账单等）。本文档聚焦上游绑定切换，数据迁移器聚焦业务数据导入，两者互补。
- [本地 IDC 数据迁移流程](../../参考资料/数据库/本地IDC数据迁移流程.md)：当前结构内 dump 迁移的正式流程。
