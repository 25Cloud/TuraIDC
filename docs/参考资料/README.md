...
| 数据库 | [从智简魔方财务系统迁移](数据库/从智简魔方财务系统迁移.md)                 | current      | 智简魔方财务（ZJMF）→ TuraIDC 完整迁移教程（含生产踩坑记录）。 |
| 数据库 | [从智简魔方财务系统迁移（实验）](数据库/从智简魔方财务系统迁移（实验）.md) | experimental | 魔方财务 shd\_ 前缀数据迁移实验工具（已归档，见正式指南）。    |
