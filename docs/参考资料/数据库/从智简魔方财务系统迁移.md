...
> 旧实验版文档见 [从智简魔方财务系统迁移（实验）](./从智简魔方财务系统迁移（实验）.md)，本文档已涵盖其全部内容并按生产经验修订（如 pricing 扁平格式）。
...
- Redis 密码、队列 worker、crontab 按 [部署与调度指南](../运维/部署与调度指南.md) 核对。
...
- [本地 IDC 数据迁移流程](./本地IDC数据迁移流程.md)
- [宝塔部署项目指南](../运维/宝塔部署项目指南.md)
- [Docker 与 1Panel 部署指南](../运维/Docker与1Panel部署指南.md)
- [部署与调度指南](../运维/部署与调度指南.md)
- [ZJMF 统一迁移方案](../../执行计划/已完成/Zjmf统一迁移方案.md)
