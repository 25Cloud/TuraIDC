...
| 脚本                                                                                                              | 作用                                                       |
| ----------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------- |
| [backend/scripts/mofang_to_turaidc_migrator.py](file:///e:/TuraIDC/backend/scripts/mofang_to_turaidc_migrator.py) | 主迁移器，解析 dump、字段映射、批量写入、pricing JSON 转换 |
| [backend/scripts/normalize_mofang_dump.py](file:///e:/TuraIDC/backend/scripts/normalize_mofang_dump.py)           | 预处理 dump，剥离 `shd_` 等前缀并重命名冲突表              |
| [backend/scripts/count_mofang_rows.py](file:///e:/TuraIDC/backend/scripts/count_mofang_rows.py)                   | 统计源表行数，评估迁移规模                                 |
| [backend/scripts/peek_mofang_rows.py](file:///e:/TuraIDC/backend/scripts/peek_mofang_rows.py)                     | 抽样源数据，验证字段格式（如 `###md5` 密码）               |
| [backend/scripts/dryrun_migration_analysis.py](file:///e:/TuraIDC/backend/scripts/dryrun_migration_analysis.py)   | 纯 Python dry-run，分析源/目标列交集和必填字段缺失         |
| [backend/scripts/extract_mofang_core_tables.py](file:///e:/TuraIDC/backend/scripts/extract_mofang_core_tables.py) | 从大 dump 中抽取核心表，便于离线分析                       |
...
魔方财务密码格式为 `###md5` 前缀 + 32 位 md5，与图拉云 bcrypt 不兼容。迁移时**原样保留**，由 [backend/plugins/servers/zjmf_finance/lib/ZjmfLegacyPasswordVerifier.php](file:///e:/TuraIDC/backend/plugins/servers/zjmf_finance/lib/ZjmfLegacyPasswordVerifier.php) 在登录时识别并验证，用户登录后可平滑升级到 bcrypt。
...
- [本地 IDC 数据迁移流程](file:///e:/TuraIDC/docs/参考资料/数据库/本地IDC数据迁移流程.md) — 正式迁移流程
- [ZJMF 统一迁移方案](file:///e:/TuraIDC/docs/执行计划/已完成/Zjmf统一迁移方案.md) — ZJMF 兼容性整体方案
